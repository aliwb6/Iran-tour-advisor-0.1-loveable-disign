            ...
            <article key={p.slug} className="group flex flex-col overflow-hidden rounded-3xl bg-card shadow-soft hover-lift">
              ...
              <div className="flex flex-1 flex-col p-7">
                ...
                <h3 className="mt-4 font-display text-2xl">
                  <Link to="/packages/$slug" params={{ slug: p.slug }} className="hover:text-accent">{t(p.titleKey)}</Link>
                </h3>
                <p className="mt-3 text-sm leading-relaxed text-muted-foreground">{t(p.descKey)}</p>
                <Link to="/packages/$slug" params={{ slug: p.slug }} className="mt-6 inline-flex items-center gap-2 text-sm font-medium text-accent">
